...
import com.example.board3.service.BoardService1;
import com.example.board3.utils.MyUtil;
...
public class BoardController {
	...
	@Autowired
	private MyUtil Utils;

	@RequestMapping(value = "/") // localhost로 접속을 하게한다. ex)localhost/
	public String index() {
		return "index";
	}
	...
	public String createdOK(BoardDTO board, HttpServletRequest request, Model model) {

		try {
			int maxNum = boardService.maxNum(); // max넘에 최댓값을 구해서 인설트할때마다 다음값으로 넘겨주기위해서

			board.setNum(maxNum + 1); // num컬럼에 들어갈 값을 1증가시켜준다

			boardService.insertDate(board);
		...
		return "redirect:/list"; // redirect는 값을 가지지않고 넘어간다.
	}

	// 페이지를 구현하는 메소드
	@RequestMapping(value = "/list", 
			method = {RequestMethod.GET, RequestMethod.POST})
	public String list(BoardDTO board, HttpServletRequest request, Model model) {
		...
		try {
			...
			if(searchValue == null) {
				searchKey = "title"; //검색 키워드의 디폴트는 subject
			...
			int totalPage = Utils.getPageCount(numPerPage, dataCount);//페이지의 전체 갯수를 구한다
			...
			//3. 전체 게시물 리스트를 가져온다.
			List<BoardDTO> lists = boardService.getLists(searchKey, searchValue, start, end); 
			...
			String pageIndexList = Utils.pageIndexList(currentPage, totalPage, listUrl);
	...
	//수정페이지 보여줌
		...
		public String updatedOK(BoardDTO board, HttpServletRequest request, Model model) {
			String pageNum = request.getParameter("pageNum"); 
			String searchKey = request.getParameter("searchKey"); 
			String searchValue = request.getParameter("searchValue"); 
			String param = "?pageNum=" + pageNum;
